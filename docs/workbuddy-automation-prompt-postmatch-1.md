# WorkBuddy 自动化任务提示词：赛后1

自动化名称必须设为：`任务三 每日赛后全流程·赛后1`
...
【本任务的固定身份】
- BATCH_ID 永远等于 post1，显示名称永远是“赛后1”。绝对不得省略命令中的 `--batch post1`。
- 本次执行窗口标题必须改为 `赛后1-YYYY-MM-DD`，日期使用本次 TARGET_DATE。最终汇报首行也必须是这个标题。
...
2. 执行：`PYTHONPATH=src .venv/bin/python -m jaguartv_postmatch.pipeline phase1 --config config/local.json --date "$TARGET_DATE" --batch post1`。
3. 读取 `runs/YYYYMMDD_post1/phase1/results.json`。如 `completed_count` 为 0，不运行 Phase 2-5，不生成假海报，不上传；按 Lark 规则发送“今日暂无官方完赛场次”与 unfinished 清单后结束。
4. 有完赛场次时执行：`PYTHONPATH=src .venv/bin/python -m jaguartv_postmatch.pipeline run --config config/local.json --date "$TARGET_DATE" --batch post1`。不得绕过 `pipeline run`手动拼接一套自创流程。

【生产与赛后1视觉硬规则】
- Phase 2 使用 agent-reach 收集当场完赛证据，专门供海报构图和提示词使用；不得用旧新闻当本场事件。
- Phase 3 必须先用当前可用大模型根据已核验证据写完整英文 Image2 提示词，再调用 Image2。
- 赛后1的强制风格是：电影级真实球场、深透视灯光、非对称球星尺度、雨/烟雾/彩带等环境纵深、下方强比分轴。禁止撕纸、报纸、贴纸、剪贴册、平面拼贴风格；这些留给赛后2。
- 同一批不同比赛也必须改变主要构图、背景、球员尺度或灯光；仅更换队伍颜色不算新设计。
...
- 海报是 4:5，成品文字只用自然巴葡。首帧和封面完整使用原海报。只生成 4 秒动态海报 hook；中间“操作类”与末尾 CTA 必须从库存轮循且完整播放，成片时长动态计算。中文文件名。
- 文案必须是赛后1独立版，包含 `Acesse jaguartvbrasil.com/baixar-app para baixar.`，标签必须包含 `#jaguartv` 和 `#iptv`。

【跨批不重复门禁】
- 运行目录必须是 `runs/YYYYMMDD_post1`，不得写入 `runs/YYYYMMDD_post2` 或无后缀目录。
- 如果 `runs/YYYYMMDD_post2` 已存在，上传前必须由仓库 Phase 5 跨批门禁比较同一 task_id 的海报 SHA-256、style、captions 和成片 SHA-256。任一重复必须停止，不得上传重复项。
- 可重用已授权的球员、队徽、球衣、操作、CTA 和音乐库存，但不得复制赛后2已生成的海报画面、整体风格、背景、成片或完整文案。
...
- 生产完成后先写本地汇报 `runs/YYYYMMDD_post1/workbuddy-report.md`，再发 Lark。消息标题固定为 `[任务三][赛后1][YYYY-MM-DD]`，内容必须包含 Git HEAD、TARGET_DATE、Task 1 fixture IDs、完赛/未完赛、海报数、视频数、上传 ID、fallback、重试、失败项和所有产物路径。
...
【最终验收】
只有在以下全部成立时才能报告完成：赛程全部来自当日 Task 1 已选清单；赛果来自 API-Football 且为 FT/AET/PEN；比分、日期、球员、胜负情绪和事件都通过验证；Figure 1 原图右上；海报 4:5 且脸部无遮挡；4 秒 hook；操作类和 CTA 完整播放；首帧/封面是完整海报；成片/文案与赛后2不重复；所有上传在 Pending Review 中仅一条且标签为“赛后比分”；Lark 有成功回执；日志和产物中无凭据。
```
